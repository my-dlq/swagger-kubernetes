...
public class KubernetesDiscovery implements SchedulingConfigurer {
    ...
    //static final String PORT_TYPE_PORT = "ClusterIP";
    ...
    public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
        taskRegistrar.addFixedRateTask(
                new IntervalTask(
                        ...
                        }, discoveryAutoConfig.getInterval() * 1000, discoveryAutoConfig.getInitialDelay() * 1000));
    }
    ...
    private void serviceFresh() {
        ...
        // 将静态服务列表加入总服务列表
        if (staticServiceList != null && staticServiceList.size() > 0) {
            if (serviceInfos != null && serviceInfos.size() > 0) {
                serviceInfos.addAll(staticServiceList);
            } else {
                serviceInfos = staticServiceList;
            }
        }
        // 验证服务列表是否有值
    ...
    private static List<ServiceInfo> getServiceInfo(String namespace, String portType, String host, String swaggerUrl) {
        ...
        for (V1Service service : serviceList) {
            // 设置 host
            String serviceHost = "http://" + service.getMetadata().getName();
            ...
            for (Integer port : ports) {
                if (!HttpUtils.checkUrl(serviceHost + ":" + port + swaggerUrl)) {
                    continue;
                }
    ...
    private static List<V1Service> getKubernetesServiceList(String namespace, String portType) {
        ...
        for (V1Service service : serviceList.getItems()) {
            ...
            if (StringUtils.equalsIgnoreCase(portType, PORT_TYPE_NODEPORT) &&
