...
public class KubernetesConnect {
    ...
    public void connection() {
        ...
        if (StringUtils.isNotEmpty(tokenPath) && StringUtils.isNotEmpty(url)) {
            ...
            token = FileUtils.readFile(new File(tokenPath), "UTF-8");
    ...
    private void connectFromCluster() {
        ...
        try {
            ...
            // if namespace is null,read the cluster environment for namespace
            String namespace = FileUtils.readFile(new File(Config.DEFAULT_FALLBACK_HOST + "/namespace"), "UTF-8");
            // Set the configuration to the spring context object
